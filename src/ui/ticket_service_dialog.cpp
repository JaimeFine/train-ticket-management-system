...
void TicketServiceDialog::setupSearchTab()
{
    ...
    auto *scheduleHint = new QLabel(
        QStringLiteral("选择一班列车后，可继续办理订票。"), tab);
    auto *bookButton = new QPushButton(QStringLiteral("预订选中车次"), tab);
...
void TicketServiceDialog::searchTrains()
{
    ...
    if (!m_trainNumberEdit->text().trimmed().isEmpty()) {
        ...
        if (!travelDate.isEmpty() && !results.isEmpty()
            && !results.first().value(QStringLiteral("departureTime"))
                    .toString().startsWith(travelDate)) {
            results.clear();
        }
    } else {
    ...
    for (int row = 0; row < results.size(); ++row) {
        ...
        m_searchResultsTable->setItem(row, 6, new QTableWidgetItem(durationText(
            result.value(QStringLiteral("travelMinutes")).toInt())));
        m_searchResultsTable->setItem(row, 7, new QTableWidgetItem(QString::number(result.value(QStringLiteral("remainingSeats")).toInt())));
