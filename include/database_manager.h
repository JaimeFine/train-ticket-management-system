...
public:
    ...
    bool initialize();     // 初始化数据库（V1 建表逻辑）
    bool initializeV2();   // V2：使用 schema_v2.sql 建表
    ...
    std::optional<TripRecord> findOrCreateTrip(int trainId, const QString &travelDate, int totalSeats);   // 不存在则按车次模板惰性创建
    ...
    // ── Transaction：事务控制（购票/改签等复合操作需原子提交）──
    ...
    struct TrainWithStations {
        int trainId=0,totalSeats=0;                                    // 车次ID、总座位数
        QString trainNumber,departureStationName,arrivalStationName;   // 车次号、出发/到达站名
        QString departureTime,arrivalTime;                             // 发车/到达时间
        int tripId=0,remainingSeats=0;                                 // 班次ID、余票数
        QString travelDate;                                            // 出行日期
        bool enabled=true;                                             // 是否在售
    };
    // 按出发站/到达站/日期联查可售班次
